...
# output from kraken2-bracken, combined to one file
  krak2_brack_file="../input/fhi__redch_krakenStnd_abundances.tsv"

# output from single kraken2 report, converted to MPA
  krak2_mpa_file="../input/fhi__redch_krakenStnd_abundances.tsv"
...
## congeal  -------------------------------------------
  ...
  # make relative abudance
  ka_relab <- apply( ka_counts, 2, function(aa){ aa/sum(aa) })
...
```{R, eval=FALSE}
## general metadata
    write.table(mgdat, "./fhi__redch__metadata-20__dat.tsv")
    saveRDS( mgdat, "./fhi__redch__metadata-20__dat.RDS")


## for Kraken2:
  
  ## tsv output
    write.table(k2_counts, "./fhi__redch__krakenStnd-20__num.tsv")
    write.table(k2_relab, "./fhi__redch__krakenStnd-20__frac.tsv")
    write.table(k2_tax, "./fhi__redch__krakenStnd-20__tax.tsv")
    
  ## RDS for R output - note samples are ROWS
    saveRDS( t(k2_counts), "./fhi__redch__krakenStnd-20__num.RDS")
    saveRDS( t(k2_relab), "./fhi__redch__krakenStnd-20__frac.RDS")
    saveRDS( k2_tax, "./fhi__redch__krakenStnd-20__tax.RDS")

        
## for Kaiju:

  ## tsv output
    write.table(ka_counts, "./fhi__redch__kaijuStnd-20__num.tsv")
    write.table(ka_relab, "./fhi__redch__kaijuStnd-20__frac.tsv")
    write.table(ka_tax, "./fhi__redch__kaijuStnd-20__tax.tsv")
    
  ## RDS for R output - note samples are ROWS
    saveRDS( t(ka_counts), "./fhi__redch__kaijuStnd-20__num.RDS")
    saveRDS( t(ka_relab), "./fhi__redch__kaijuStnd-20__frac.RDS")
    saveRDS( ka_tax, "./fhi__redch__kaijuStnd-20__tax.RDS")
...
## Kraken2/Bracken   ---
  ...
  # look at the first ten rows, ten columns:
    ...
    (kr2_phyloseq_dat <- phyloseq(mg_count, mg_tax, mgdat))

    saveRDS( kr2_phyloseq_dat, "./fhi__redch__KrakenStnd__phylo.RDS")
    phylo1 <- readRDS("./fhi__redch__KrakenStnd__phylo.RDS")
    phylo1
...
## Kaiju   ----------
  ...
  # note: we use the count data..
    (kai_phyloseq_dat <- phyloseq(mg_count, mg_tax, mgdat))

    saveRDS( kai_phyloseq_dat, "./fhi__redch__KaijuStnd__phylo.RDS")
    phylo2 <- readRDS("./fhi__redch__KaijuStnd__phylo.RDS")
    phylo2
