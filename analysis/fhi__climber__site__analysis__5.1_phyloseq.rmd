---
title: 'Getting Metagenomics into `R` - extra steps in `phyloseq`'
author: 'jfg'
...
output:
  ...
  pdf_document: 
    ...
    toc_depth: 3
    number_sections: FALSE
knit: (function(inputFile, encoding) { rmarkdown::render(inputFile, encoding=encoding, output_file='../documents/R_to_phyloseq.html') })
---

> exceedingly basic workflow `r emo::ji("construction")`

---

# Introduction

Following from work [assembling the metagenomic data](./shotgun_assembly.html), and [importing the microbial community into an `R`-friendly format](./data_to_R.html), we might want to use the handy [phyloseq](https://joey711.github.io/phyloseq/import-data.html) package for `R` to look at things.
...
It was linked in the last page, but the teams behind `DADA2` and `phyloseq` put together [an incredible showcase paper of what can be done](https://f1000research.com/articles/5-1492): do check it out.
...
This really is only for reference - these commands are simply ripped straight from the excellent [phyloseq](https://joey711.github.io/phyloseq/import-data.html) tutorial section, as this package covers many of the important first steps. For more info, please see the tutorial section linked above.
