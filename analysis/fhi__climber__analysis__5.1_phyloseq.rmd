...
Following from work assembling the [metagenomic data](./shotgun_assembly.html), and importing the microbial community into `R`-friendly format.
...
From our first workflow, we should have a *feature count table*:

We should also have a *taxonomy table*:

We should hopefully have (or can build) a *sample data table* also - this will tell us about the samples we're analysinig (i.e. metadata):
